...
//Pins
const byte CS_pin = 7;	//goes to CS on ADS1256
...
void loop()
{
    ...
    case '0': // Kirim '0' untuk mematikan Pin 7
      setCurrentMode(false);
      break;

    case '1': // Kirim '1' untuk menyalakan Pin 7
      setCurrentMode(true);
      break;
...
void setCurrentMode(bool highCurrent) 
{
  if (highCurrent) 
  {
    digitalWrite(RELAY_PIN, HIGH);
    Serial.println("*Mengubah mode arus karena pembacaan tegangan dibawah 1 mV");
    Serial.println("*RELAY ON: Mode Arus Tinggi (Pembagi 0.1)");
  } 
  else 
  {
    digitalWrite(RELAY_PIN, LOW);
    Serial.println("*Mengubah mode arus karena disipasi daya diatas 5 mW");
    Serial.println("*RELAY OFF: Mode Arus Rendah (Pembagi 0.004)");
  }
  // Delay opsional untuk settling time relay/arus jika diperlukan
  delay(100); 
}
...
void readSingle()
{
  ...
  //SPI.transfer(0x0F) probably also takes some time, therefore less delay is enough.
   ...
   Serial.println("Voltage Reading (V):");
...
void readSingleContinuous() //Continuously reading 1 single-ended channel (i.e. A0+A1)
{
  //Relevant video: https://youtu.be/4-A8aJ5BzIs
...
void readContinuous_AIN0_AIN1() 
{
  ...
  while (Serial.read() != 's') 
  {
    ...
    Serial.println("Resistance Reading (Ohm):");
...
void voltageToResistance(int32_t registerData)
{
  // 1. Konversi Data Mentah ke Tegangan
  if (long minus = registerData >> 23 == 1)
  {
  ...
  // 2. Logika Perhitungan Resistansi Berdasarkan Status Pin 7
  // Menggunakan digitalRead untuk mengecek status fisik pin saat ini
  if (digitalRead(RELAY_PIN) == HIGH) 
  {
    // Jika Pin 7 ON (High Current Mode)
    resistance = voltage / 0.1; // Asumsi arus 100mA (0.1A)
  } 
  else 
  {
    // Jika Pin 7 OFF (Low Current Mode)
    resistance = voltage / 0.004; // Arus 4mA (0.004A)
  }

  // 3. Print Hasil
  Serial.println(resistance, 4); // Menampilkan resistansi dengan 4 desimal
...
{
    Serial.println("*--- Initiating SPI Sanity Check ---");

    // 1. Check STATUS Register
    // The ADS1256 STATUS register contains the ID bits in the upper nibble.
    // Default value is usually 0x31. Your code sets it to B00110110 (0x36).
    uint8_t statusVal = readRegister(0x00);
    ...
    {
        Serial.println("*FAILURE: SPI communication is dead. MISO is stuck LOW (0x00) or HIGH (0xFF).");
        Serial.println("*ACTION: Check wiring on MISO, MOSI, SCK, and CS. Verify the board is powered.");
        return; // Exit early, SPI is broken
    } 
    
    // 2. Write-Read Verification
    // If SPI is partially alive, we test writing a known value and reading it back.
    Serial.println("*SPI Alive. Testing Register Write/Read Integrity...");
    
    uint8_t originalMux = readRegister(0x01); // Store current MUX state
    
    writeRegister(0x01, B00001000); // Write AIN0+AINCOM 
    delay(10); // Brief delay for register update
    
    uint8_t testMux = readRegister(0x01);
    
    if (testMux == B00001000) 
    {
    ...
    writeRegister(0x01, originalMux); // Restore original state
